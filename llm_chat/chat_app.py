...
# Select the model
#selected_model = model_selector(client=ollama_client)
...
if st.session_state.current_chat_id:
    ...
    if chat_name not in st.session_state.chats or not chat_name:
        #st.session_state.chats[chat_name] = {"role": "system", "content": "You are acting as a software architect answering questions for the Principal software architect.  You provide details when needed but generally give concise answers.  You are an expert in Java, OSGi, and Intermediate at Python tasks. You also give very creative answers."}
        ...
        if st.session_state.selected_system_prompt:
            ...
            st.write(st.session_state.chats[chat_name])
...
if prompt := st.chat_input("What would you like to say?"):
    ...
    if chat_name not in st.session_state.chats or not chat_name:
        ...
        #st.session_state.chats[chat_name] = {"role": "system", "content": "You are acting as a software architect answering questions for the Principal software architect.  You provide details when needed but generally give concise answers.  You are an expert in Java, OSGi, and Intermediate at Python tasks. You also give very creative answers."}
        st.session_state.chats[chat_name] = ( [{"role": role, "content": content} for role, content in messages])
        st.session_state.chats[chat_name].insert(0,{"role": "system", "content": "You are acting as a software architect answering questions for the Principal software architect.  You provide details when needed but generally give concise answers.  You are an expert in Java, OSGi, and Intermediate at Python tasks. You also give very creative answers."})
